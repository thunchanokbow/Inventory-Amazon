...
2. Create the storage in GCS to store the data from PostgreSQL database, input and output from Spark jobs. Using airflow operators PostgresToGSCOperator, DataprocSubmitJobOperator.
3. Create Cloud Composer environment managed Apache Airflow in Google Cloud Platfrom. 
4. Provide the Airflow UI Admin connection database credentials, PyPI packages Pandas in GCC.
5. Upload Spark jobs file AND DAG file to Google Cloud Storage.
6. Create a Google Cloud Dataproc cluster to run Spark jobs. Using airflow operators ClusterGenerator, DataprocCreateClusterOperator, DataprocDeleteClusterOperator.
7. Storing the results of a Spark job in BigQuery, Using BashOperator.
8. Delete the data in the Google Cloud Storage bucket.
"""
